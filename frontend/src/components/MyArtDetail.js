...
//           <img src={`http://localhost:3001/uploads/${details.img}`} alt="painting" className=' object-fill'/>
//         </div>
...
const MyArtDetail = ({ details }) => {
  ...
  const [updatedDescription, setUpdatedDescription] = useState(details.description);
...
//   const [updatedImage, setUpdatedImage] = useState(details.image);
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const response = await axios.put(`http://localhost:3001/updateArt/${details.id}`, updatedData);
  ...
  const handleDelete = async () => {
    try {
      const response = await axios.delete(`http://localhost:3001/deleteArt/${details.id}`);
  ...
  return (
    ...
    {/* Toggle edit mode */}
    {!editMode ? (
      <button className='bg-blue-500 text-white font-bold py-2 px-4 rounded mt-2' onClick={() => setEditMode(true)}>
        Edit
      </button>
    ) : (
      <>
        <button className='bg-blue-500 text-white font-bold py-2 px-4 rounded mt-2' onClick={handleUpdate}>
          Update
        </button>
        <button className='bg-gray-500 text-white font-bold py-2 px-4 rounded mt-2' onClick={() => setEditMode(false)}>
          Cancel
        </button>
      </>
    )}
    ...
    {/* Delete button */}
    <button className='bg-red-500 text-white font-bold py-2 px-4 rounded mt-2' onClick={handleDelete}>
      Delete
    </button>
